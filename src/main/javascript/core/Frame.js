Frame = function(classDef, methodDef, args, parent) {
	_.extend(this, Watchable);

	var _stack = new Stack();
	var _variables = new LocalVariables(args ? args : []);
	var _child;
	var _thread;
	var _output;
	var _currentInstruction;
	var _version = Frame.index++;
	var _instructions = new Iterator(methodDef.getInstructions());
	...
	this.executeChild = function(classDef, methodDef, args) {
		_child = new Frame(classDef, methodDef, args, this);
	...
	this._executeNextInstruction = function() {
		...
		try {
			var output = _currentInstruction.execute(this);
